...
from .to_evaluate import vector_is_readable,is_crs,is_join
...
def RandomForestModel(path_train,segs_path,\
        ...
        if bool(stateCheckBox) :
            ...
            #To evatualte join
            if is_join(dfjv.values,'Data set and validation samples do not overlap') == False:
                return 0
            #Criar amostras de treinamento, merge attribute spatial join
            dfjt=gpd.sjoin(dft,df_dataset,how="inner", op='intersects')
            #To evatualte join
            if is_join(dfjt.values,'Data set and training samples do not overlap') == False:
                return 0
            #criar modelo Random Forest
